...
export function CoursesPage() {
  ...
  function confirmDeleteCourse(course: Course) { if (window.confirm(`确定删除课程“${course.name}”吗？删除后不可恢复。请先处理课程下的班级，删除班级会同时永久删除其平时成绩数据。`)) deleteCourse.mutate(course); }
  function confirmDeleteClass(item: ClassGroup) { if (window.confirm(`确定删除班级“${item.name}”吗？该班级的学生名单和全部历史考勤记录将永久删除，且无法恢复。平时成绩项目、积分和换算规则也将同时永久删除。`)) deleteClass.mutate(item); }
