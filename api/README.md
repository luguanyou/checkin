...
已有环境启用本模块前，确认 `.env` 指向目标数据库，在本目录执行 `uv run alembic upgrade head`，再重启 API 并更新前端构建。迁移 `0004_add_scores` 创建成绩规则、评分项目及学生积分表，`0005_add_score_item_default_points` 增加项目默认积分字段，不修改考勤记录。
...
基础分默认 70，可按班级修改或暂时留空。项目可配置默认积分，新建项目时初始化活动学生记录；后续修改默认积分不会覆盖学生已保存的单独分值。每类净积分乘以该类别系数后加上基础分，仅最终平时成绩限制在 0～100 分；考勤不参与计算。积分及规则支持最多四位小数，API 的小数输出为字符串，正式分数保留两位小数。空积分表示未评价，汇总按 0 分计算；零积分表示已评价。
